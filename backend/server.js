...
app.get('/folder/:name', async (req, res) => {
    ...
    try {
        const data = await s3Client.listObjectsV2({ Bucket: BUCKET_NAME, Prefix: `${foldername}/` }).promise();
...
app.post("/newfolder/:frameworkname",async (req,res)=>{
      ...
      try{
          const fileData = await s3Client.listObjectsV2({ Bucket: BUCKET_NAME, Prefix: `base/${framework}/` }).promise();
...
app.post('/addFile/:framework/:folder/:filename', async(req,res)=>{
     ...
     if(newFileName){
        ...
        if(dotIndex < newFileName.length && dotIndex > 0){
            try{
                const fileData = await s3Client.listObjectsV2({ Bucket: BUCKET_NAME, Prefix: `base/${frameworkname}/` }).promise();
                let copyFileContent;
